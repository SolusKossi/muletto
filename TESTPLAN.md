...
| Video recording date from `mvhd` | `V` | `tests/fixtures/video/` - three real H.264 clips from ffmpeg, 9 KB the lot. Two carry a creation time and are read to the second; one carries none and correctly gets no date |
| The file's date beating the filename | `V` | The decisive case, built on purpose: a clip named `2019-11-02_disagrees-main.mp4` whose `mvhd` says 2020-05-05. The library dates it **2020-05-05**, so the ordering holds - what the recorder wrote beats what the name claims |
| Video poster frames | `V` | **Implemented and never run until now.** All three clips produce a JPEG at the clip's own dimensions (256x144, 320x240, 192x108). Not a blank canvas either: the frame has 3,117 distinct colours and a standard deviation of 69.7, so something was really decoded |
| Tile recycling past 400 photographs | `V` | **520 photographs, and the cap held.** 0 never decoded, 400 decoded at rest, 120 recycled back to a skeleton, 520 accounted for, 4 MB of JS heap at the end. Driven by deleting `IntersectionObserver` before the library opens, which makes explorer.js take its own documented no-observer path and decode every tile through the real `decodeCell` - and `recycle()` is called from inside `decodeCell`, not by scrolling, so the cap is exercised without needing a composited window. `tools/make-large-library.py` builds the library and prints the one line that runs it |
| Scroll-driven decoding in a real window | `-` | The other half, and still open. The run above proves the cap and the recycling; it does not prove the observer wiring, because it removes the observer to get there. Whether scrolling triggers decoding needs a window that composites |
| Parsing a 520-photograph library | `V` | 1.1 s, all 520 dated. The parse holds no DOM, so this part is honest from Node |
...
| Split captions, real counts | `V` | 126 overlays, every one paired, no orphans. 94 pair with a picture, which composites; 32 with a video, which is written beside instead. The orphan path cannot be exercised by this export because it has none |
| Caption compositing, the pixels | `V` | Measured in the browser against the sample pairs, which is where the merge actually runs. Where the caption is transparent the result differs from the memory by **0.67 on average across 368,800 pixels**, worst 21; where it is opaque it differs from the caption by **1.17 across 11,600 pixels**, worst 12. Both images are JPEG round-trips, so single-digit drift is the encoder. The memory passes through untouched and the caption lands where it was put |
| Caption compositing, mismatched sizes | `V` | The branch the sample cannot reach on its own, because both its files are 720x540 - and a real Snapchat overlay is rendered at the phone's screen size, not the camera's. Re-rendering the caption at half size and merging again gives a **720x540 result**: the caption is scaled to the memory, not the memory shrunk to the caption. Scaling the other way would throw away half the picture |
...
Read since 2026-08-28. Four of the rows
...
The rest is worth keeping whole, because the request flow itself produces a
shape nothing else does: the category dropdown takes one category at a time,
so one person's Amazon export is often several unrelated archives requested
weeks apart, and one folder can hold two incompatible versions of the same
file.
...
| Four naming conventions in one export | `S` | `Title Case With Spaces`, `PascalCaseNoSpaces`, `snake_case`, and `camelCase` in the EU JSON build |
...
| Timestamp precision varying inside one column | `S` | 1,574 rows without fractional seconds, 309 with |
| Prime Video using a different format entirely | `S` | `2019-04-08 18:48:31.276` - space separator, no `T`, no `Z`, two or three fractional digits. Audible is date-only |
| No timezone on any Amazon date | `S` | Prime Video and Audible both write without one, and Amazon does not document which zone it meant. Read as UTC so that every reader sees the same time, and said out loud - read as local, an order near midnight lands on a different day for each person |
...
| Identical filenames in different archives | `S` | `Advertising.AdvertiserAudiences.csv` exists in `Advertising.1` and `Advertising.2` with different contents. Flattening the tree silently loses one |
...
| Empty archives as a normal result | `S` | Unused services ship empty zips, and Amazon creates default empty Alexa lists for accounts that never used Alexa. File present is not data present |
| Mixed separators in top-level names | `S` | Dots, hyphens, underscores and spaces all appear: `Retail.OrderHistory.1`, `Amazon-Music`, `Alexa_1`, `Audio and Transcription` |
...
| Amazon Photos metadata with no photographs | `S` | The export documents the library and does not contain it. Another case where "explain, do not show an empty library" is the right answer |
