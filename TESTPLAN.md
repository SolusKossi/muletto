...
| Tile recycling past 400 photographs | `-` | **Blocked, and worth saying why rather than leaving it blank.** `tools/make-large-library.py` builds a 520-photograph library and writes it, with a measuring page, into the gitignored scratch directory it names. The measurement cannot be taken here: the grid recycles through an `IntersectionObserver`, and in a preview pane that is not compositing frames that observer never fires - measured, with `requestAnimationFrame` and `requestIdleCallback`, all three silent. A count taken there reads as "nothing recycled", which is also what a broken grid reads as. Needs a real window |
